...
<div class="container">
  <main>
    ...
    <h2>🎓 Tips for University</h2>
    <p class="subtitle">เคล็ดลับการเรียนและชีวิตในรั้วมหาวิทยาลัย</p>

    {#if posts.filter(p => p.type === 'Tips').length === 0}
      <div class="empty">ยังไม่มี Tips ที่โพสต์โดย Host</div>
    {:else}
      {#each posts.filter(p => p.type === 'Tips') as post}
        <div class="post-card">
          ...
          {#if post.fileURL}
            {#if post.isImage}
              <img src={post.fileURL} alt="Tip Image" />
            {:else}
