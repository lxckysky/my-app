<script lang="ts">
  ...
  // ✅ ดึงรูปจาก localStorage ตาม role
  let usersKey = role === 'host' ? 'users_host' : 'users_parasite';
  const users = JSON.parse(localStorage.getItem(usersKey) || '[]');
  const found = users.find((u: any) => u.email === email);

  profilePic = found?.profilePic || '/icons/avatar.png';
...
<div class="profile-container">
  ...
  <h2 class="level-display">🎮 LEVEL {level}</h2>
  <div class="xp-bar-container">
    <p class="xp-text">🔥 {xp} / 1000 XP</p>
    <div class="xp-bar">
  ...
  <button class="record-btn">My Record Book</button>
    ...
    <p><strong>Role:</strong> Challenger</p>
